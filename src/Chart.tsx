...
import { DEFAULT_FORMAT, DEFAULT_VISUAL, THRESHOLD_PLOT_INTERVAL, TimeSeriesChartOptions } from './model';
import {
  convertPanelYAxis,
...
interface PanelProps {
  definition: PanelDefinition<TimeSeriesChartOptions>;
...
export default function Panel(props: PanelProps) {
  const {
    definition: {
      spec: {
        plugin: {
          spec: { thresholds, yAxis, tooltip, legend: legenFromProps, visual: visualFromProps },
        },
      },
    },
    contentDimensions,
  ...
  const legend =
    legenFromProps && validateLegendSpec(legenFromProps) ? merge({}, DEFAULT_LEGEND, legenFromProps) : undefined;

  // TODO: add support for y_axis_alt.format
  const format = yAxis?.format ?? DEFAULT_FORMAT;

  // ensures there are fallbacks for unset properties since most
  // users should not need to customize visual display
  const visual = merge({}, DEFAULT_VISUAL, visualFromProps);
