...
import { TimeSeriesChartVisualOptions } from '../model';
...
export function getSeriesColor(props: SeriesColorProps) {
  const { categoricalPalette, visual, seriesName, seriesIndex, totalSeries } = props;
...
const colorGenerator = new ColorHash({
  hue: { min: ERROR_HUE_CUTOFF, max: 360 },
});
const redColorGenerator = new ColorHash({
  hue: { min: 0, max: ERROR_HUE_CUTOFF },
});
